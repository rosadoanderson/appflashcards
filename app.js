...
function mediaCandidates(name) {
  const encoded = encodeURIComponent(name).replace(/%2F/g, "/");
  return PASTAS_MEDIA.map(base => base + encoded);
}
function resolveMediaPath(name, onSuccess, onFailure) {
  ...
  probe.onload = () => { MEDIA_PATH_CACHE.set(normalized, candidates[index]); onSuccess(candidates[index]); };
  probe.onerror = () => {
    index += 1;
...
async function boot() {
  ...
  state.cards = baseParts.flat().map(card => ({ ...card, stats: savedStats[card.id] || defaultCardStats(), shortDeck: shortDeckName(card.deck), searchText: `${card.front || ""} ${card.back || ""} ${card.deck || ""} ${card.title || ""}`.toLowerCase() }));
  bindEvents();
  renderAll();
  ...
  if (window.caches?.keys) {
    try {
      ...
      await Promise.all(keys.filter(key => key.startsWith("anking-")).map(key => caches.delete(key)));
    } catch {}
  }
