...
const PASTAS_MEDIA = ["media/"] .concat(Array.from({ length: 24 }, (_, i) => `media/media_${i + 1}/`));
const state = { cards: [], selectedDeck: "", studyQueue: [], studyIndex: 0, revealed: false, searchPage: 1, sessionSeconds: 0, sessionDeck: "", timerHandle: null, deckTime: {}, totalTime: 0, dailyGoal: 50, reviewLog: {} };
...
function encodeMediaSegment(value) { return value.split('/').map(part => encodeURIComponent(part)).join('/'); }
function candidateMediaPaths(original) {
  const clean = (original || '').replace(/^\.\//, '').trim();
  if (!clean.startsWith("media/")) return [];
  const tail = clean.slice("media/".length);
  const baseName = tail.split('/').pop();
  const encodedTail = encodeMediaSegment(tail);
  const encodedBaseName = encodeURIComponent(baseName);
  const set = new Set();
  const push = (v) => { if (v) set.add(v); };
  push(clean);
  push(encodeMediaSegment(clean));
  for (const folder of PASTAS_MEDIA) {
    push(folder + tail);
    push(folder + encodedTail);
    push(folder + baseName);
    push(folder + encodedBaseName);
  }
  return [...set];
}
function resolveMediaElement(node, loader) {
  const original = node.getAttribute('data-original-src') || node.getAttribute('src') || '';
  if (!original.startsWith('media/')) return;
  node.setAttribute('data-original-src', original);
  const candidates = candidateMediaPaths(original);
  let idx = 0;
  const tryNext = () => {
...
async function boot() {
  ...
  state.cards = baseParts.flat().map(card => ({ ...card, stats: savedStats[card.id] || defaultCardStats() }));
  bindEvents();
  renderAll();
  if ("serviceWorker" in navigator) { window.addEventListener("load", async () => { try { const reg = await navigator.serviceWorker.register(`./service-worker.js?v=${APP_VERSION}`); if (reg.update) reg.update().catch(() => {}); } catch (_) {} }); }
...
function bindEvents() {
  ...
  el("studyDeckSelect").addEventListener("change", () => { state.selectedDeck = el("studyDeckSelect").value || ""; renderAll(); });
  ...
  el("showAnswerBtn").addEventListener("click", () => { state.revealed = true; renderStudyCard(); });
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (el("studyCard").classList.contains("hidden")) return;
    if (event.code === "Space") { event.preventDefault(); if (!state.revealed) { state.revealed = true; renderStudyCard(); } }
    if (!state.revealed) return;
    if (event.key === "1") answerCard("again");
    if (event.key === "2") answerCard("hard");
    if (event.key === "3") answerCard("good");
    if (event.key === "4") answerCard("easy");
  });
...
function renderSubjects() {
  ...
  el("subjectCardsGrid").innerHTML = decks.map(deck => { const cards = state.cards.filter(c => c.deck === deck); const totals = computeTotals(cards); const wrongOnly = cards.filter(hasWrong).length; const reviewedTodayCount = cards.filter(reviewedToday).length; const queues = countQueues(cards); const attempts = totals.correct + totals.wrong; const acc = attempts ? Math.round((totals.correct / attempts) * 100) : 0; return `<div class="subject-card"><h3>${escapeHtml(deck)}</h3><div class="subject-meta"><span class="tag">${cards.length.toLocaleString("pt-BR")} cards</span><span class="tag">${queues.due.toLocaleString("pt-BR")} disponíveis</span><span class="tag">${wrongOnly.toLocaleString("pt-BR")} erradas</span><span class="tag">${reviewedTodayCount.toLocaleString("pt-BR")} hoje</span><span class="tag">${formatSeconds(state.deckTime[deck] || 0)}</span><span class="tag">${acc}% retenção</span></div><div class="toolbar"><button class="btn btn-secondary subject-open" data-deck="${escapeHtmlAttr(deck)}">Disponíveis</button><button class="btn btn-primary subject-random" data-deck="${escapeHtmlAttr(deck)}">Diário 20</button><button class="btn btn-danger subject-wrong" data-deck="${escapeHtmlAttr(deck)}">Erradas</button><button class="btn btn-secondary subject-today" data-deck="${escapeHtmlAttr(deck)}">Hoje</button></div></div>`; }).join("") || `<div class="panel">Nenhuma matéria encontrada.</div>`;
...
function answerCard(grade) { const queueCard = state.studyQueue[state.studyIndex]; if (!queueCard) return; const realCard = state.cards.find(c => c.id === queueCard.id) || queueCard; applySm2(realCard, grade); addReviewLog(); saveStats(); state.studyIndex += 1; state.revealed = false; renderStudyCard(); renderDashboard(); renderSubjects(); renderStats(); }
function renderSearch() { const query = (el("cardSearchInput").value || "").trim().toLowerCase(); const deck = el("searchDeckSelect").value || state.selectedDeck || ""; const queueFilter = el("searchQueueSelect").value || ""; let cards = state.cards; if (deck) cards = cards.filter(card => card.deck === deck); if (queueFilter === "new") cards = cards.filter(c => queueName(c) === "new"); else if (queueFilter === "learning") cards = cards.filter(c => queueName(c) === "learning"); else if (queueFilter === "review") cards = cards.filter(c => queueName(c) === "review"); else if (queueFilter === "wrong") cards = cards.filter(hasWrong); else if (queueFilter === "today") cards = cards.filter(reviewedToday); if (query) cards = cards.filter(card => card.front.toLowerCase().includes(query) || card.back.toLowerCase().includes(query) || card.deck.toLowerCase().includes(query) || card.title.toLowerCase().includes(query)); el("searchCountText").textContent = cards.length.toLocaleString("pt-BR") + " resultados"; const totalPages = Math.max(1, Math.ceil(cards.length / PAGE_SIZE)); if (state.searchPage > totalPages) state.searchPage = totalPages; const start = (state.searchPage - 1) * PAGE_SIZE; const pageCards = cards.slice(start, start + PAGE_SIZE); el("searchResults").innerHTML = pageCards.map(card => `<div class="result-item"><div class="toolbar"><span class="tag">${escapeHtml(card.deck)}</span><span class="tag">${nomeFila(card)}</span><span class="tag">${isDue(card) ? "disponível" : "agendado"}</span></div><div class="result-front">${escapeHtml(card.title || card.deck.split("::").slice(-1)[0])}</div><div class="result-back">${card.frontHtml || escapeHtml(card.front)}</div><div class="muted">Respondido: ${(card.stats.reviewed || 0).toLocaleString("pt-BR")} • Acertos: ${(card.stats.correct || 0).toLocaleString("pt-BR")} • Erros: ${(card.stats.wrong || 0).toLocaleString("pt-BR")} • Disponível: ${formatDateShort(card.stats?.srs?.dueAt || 0)}</div></div>`).join("") || `<div class="muted">Nenhum resultado encontrado.</div>`; resolveMediaIn(el("searchResults")); const pagesToShow = []; for (let i = 1; i <= totalPages; i++) if (i === 1 || i === totalPages || Math.abs(i - state.searchPage) <= 2) pagesToShow.push(i); el("searchPagination").innerHTML = [...new Set(pagesToShow)].map(page => `<button class="page-btn ${page === state.searchPage ? "active" : ""}" data-page="${page}">${page}</button>`).join(""); document.querySelectorAll("#searchPagination .page-btn").forEach(btn => btn.addEventListener("click", () => { state.searchPage = Number(btn.dataset.page); renderSearch(); })); }
function renderStats() { const cards = filteredCards(); const totals = computeTotals(cards); const attempts = totals.correct + totals.wrong; const accuracy = attempts ? Math.round((totals.correct / attempts) * 100) : 0; const wrongOnly = cards.filter(hasWrong).length; const queues = countQueues(cards); const currentDeckTime = state.selectedDeck ? formatSeconds(state.deckTime[state.selectedDeck] || 0) : formatSeconds(state.totalTime); const previsaoRetencao = Math.max(0, Math.min(100, Math.round(accuracy * 0.85 + (queues.review ? 10 : 0)))); el("statsSummary").innerHTML = [["Cards no filtro", cards.length.toLocaleString("pt-BR")],["Revisões", totals.reviewed.toLocaleString("pt-BR")],["Acertos", totals.correct.toLocaleString("pt-BR")],["Erros", totals.wrong.toLocaleString("pt-BR")],["Erradas ativas", wrongOnly.toLocaleString("pt-BR")],["Retenção atual", accuracy + "%"],["Previsão de retenção", previsaoRetencao + "%"],["novos", queues.new.toLocaleString("pt-BR")],["aprendendo", queues.learning.toLocaleString("pt-BR")],["revisão", queues.review.toLocaleString("pt-BR")],["disponíveis", queues.due.toLocaleString("pt-BR")],["Tempo no filtro", currentDeckTime]].map(([label, value]) => `<div class="summary-item"><div class="muted">${label}</div><strong>${value}</strong></div>`).join(""); const byDeck = {}; state.cards.forEach(card => { if (!byDeck[card.deck]) byDeck[card.deck] = { total:0, reviewed:0, wrong:0 }; byDeck[card.deck].total++; byDeck[card.deck].reviewed += card.stats.reviewed || 0; byDeck[card.deck].wrong += card.stats.wrong || 0; }); const topDecks = Object.entries(byDeck).sort((a,b) => b[1].total - a[1].total).slice(0,12); el("topDecks").innerHTML = topDecks.map(([deck, info], idx) => `<div class="rank-item"><div class="muted">#${idx + 1}</div><div><strong>${escapeHtml(deck)}</strong></div><div class="muted">${info.total.toLocaleString("pt-BR")} cards • ${info.reviewed.toLocaleString("pt-BR")} revisões</div></div>`).join(""); const timeRows = Object.entries(state.deckTime).sort((a,b)=>b[1]-a[1]).slice(0,20); el("timePerDeckList").innerHTML = timeRows.map(([deck, sec], idx) => `<div class="rank-item"><div class="muted">#${idx + 1}</div><div><strong>${escapeHtml(deck === "__all__" ? "Sessões com todos os decks" : deck)}</strong></div><div class="muted">${formatSeconds(sec)}</div></div>`).join("") || `<div class="muted">Ainda não há tempo registrado.</div>`; const worstRows = Object.entries(byDeck).sort((a,b)=>b[1].wrong-a[1].wrong).slice(0,20); el("worstDecksList").innerHTML = worstRows.map(([deck, info], idx) => `<div class="rank-item"><div class="muted">#${idx + 1}</div><div><strong>${escapeHtml(deck)}</strong></div><div class="muted">${info.wrong.toLocaleString("pt-BR")} erro(s)</div></div>`).join("") || `<div class="muted">Ainda não há erros registrados.</div>`; }
